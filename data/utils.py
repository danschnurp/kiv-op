...
def sanitize_html_for_web(value, display_code=True):
    ...
    for tag in soup.find_all(True):
        if tag.name not in VALID_TAGS:
            ...
            if tag.parent.name == "pre" and tag.string is not None:
                tag.string.replace_with("Inserted code --- see details for code expansion")

    return str(soup)


def sanitize_html_for_index(value):
    """
    Sanitize HTML from all unwanted HTML tags. All <pre><code>...</code></pre>
    sections are replaced with placeholder text is display_code is set to False
    ...
    question_code = ""
    for tag in soup.find_all(True):
        if tag.name == "code" and tag.parent is not None and tag.parent.name == "pre":
            question_code += f" {tag.string}"
        if tag.name == "p":
            text += f" {tag.string}"
    return f"<CLS>{text}<SEP>{question_code}<SEP>"
